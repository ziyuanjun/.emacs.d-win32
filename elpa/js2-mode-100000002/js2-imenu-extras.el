     ...
     (unless end-p
       (cond
        ...
        ((and js2-imenu-show-other-functions
         ...
         ;; Node is `js2-function-node' (function definition),
         ;; AND parent of node is function call:
         ;;   setTimeout(function() { ... }, 100})
         (js2-imenu-record-orphan-call-node-function node))
...
(defun js2-imenu-record-orphan-call-node-function (fn-node)
  "Record orphan function when it's the value of FN-NODE.
FN-NODE must be `js2-function-node' and parent of FN-NODE be `js2-call-node'."
  (let* ((parent (js2-node-parent fn-node))
         chain)
    (when (and parent (js2-call-node-p parent)
               (not (and js2-imenu-function-map
                         (gethash fn-node js2-imenu-function-map))))
      ;; target passed to `js2-compute-nested-prop-get' could be:
      ;;     setTimeout(function(){...}, 100); // js2-name-node
      ;;     a.then(function() {}); // js2-prop.get-node
      (setq chain (js2-compute-nested-prop-get (js2-call-node-target parent)
                                               (js2-call-node-args parent)))
      (push js2-imenu-other-functions-ns chain)
      (js2-record-imenu-entry fn-node
                              chain
                              (js2-node-abs-pos fn-node)))))
